...
epochs = 20
num_workers = 4
img_data_dir = '/vol/biomedic3/bglocker/msc2023/sea22/datafiles/chexpert/'
...
class DenseNet(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_disease, _, _ = self.process_batch(batch)
        optim_disease.zero_grad()
        self.manual_backward(loss_disease)
        optim_disease.step()

        _, _, loss_race = self.process_batch(batch)
        optim_race.zero_grad()
        self.manual_backward(loss_race)
        optim_race.step()
      
        _, loss_sex, _ = self.process_batch(batch)
        optim_sex.zero_grad()
...
def main(hparams):
    ...
    # data
    data = CheXpertDataModule(csv_train_img='/vol/biomedic3/bglocker/msc2023/sea22/datafiles/chexpert/CheXpert-v1.0/chexpert.sample.train.csv',
